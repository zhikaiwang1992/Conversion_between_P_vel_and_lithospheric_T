function [Vsyn, Tsyn, K, U, D] = Goes_convert_vel_to_temperature(Vobs, Pref, Tref, Kref, Uref, Dref, Ptarg, Tinit, alpha0, ...
                                                                  dK_dP, dK_dT, dU_dP, dU_dT, Fdamp, A, a, d, V, H, R, omega, vps, dTmax,dMPTdenp, Anelas)

%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Goes_convert_vel_to_temperature.m
%%
%%  Function: Convert the velocity from tomography to temperature given a reference velocity at the
%%            reference pressure and temperature using the model proposed by Goes and Govers (2000,JGR).
%%
%%  Zhikai WANG, zwang@ipgp.fr 
%%
%% last change at 09/03/2019
%%
...
%% Step 5: update the temperature at a specfic position (Ptarg)
while(abs(Fdamp * ((Vobs-Vsyn)/(dV_dT)))> dTmax)
    Fdamp = Fdamp/ 2;
end
Tsyn = Tinit + Fdamp * ((Vobs-Vsyn)/(dV_dT));
